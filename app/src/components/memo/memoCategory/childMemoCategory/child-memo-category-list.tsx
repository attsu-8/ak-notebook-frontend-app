...
import {
  ...
  fetchAsyncCreateChildMemoCategory,
  changeSelectChildMemoCategory,
  selectSelectParentMemoCategory,
  fetchAsyncLogicalDeleteChildMemoCategory,
  fetchAsyncPatchChildMemoCategory,
  selectIsChildMemoCategoryNewEditorOpen,
...
export const ChildMemoCategory: VFC = () => {
  ...
  const [isUpdateChildMemoCategoryOpen, setIsUpdateChildMemoCategoryOpen] =
    useState<boolean>(false);
  const [isDeleteChildMemoCategoryOpen, setIsDeleteChildMemoCategoryOpen] =
    useState<boolean>(false);
  ...
  const onClickDeleteButton = async (childMemoCategoryId: string) => {
    const result: any = await dispatch(
      fetchAsyncLogicalDeleteChildMemoCategory(childMemoCategoryId),
    );
  ...
  return (
    <>
      ...
      >
        <Box>
          <Typography color='textSecondary' variant='h6' align='center' sx={{ py: 1 }}>
            子カテゴリ
          </Typography>
          <Divider />
        </Box>
        {isSelectParentMemoCategory ? (
          <>
            ...
            >
              <MemoAddButton toolTipTitle='新規追加' onClickAction={onClickAddButton}>
                <Box sx={{ mx: 1 }}>
      ...
      <ChildMemoCategoryEditorDialog
        ...
        footerButton={
          <MemoSubmitButton form='newChild' toolTipTitle='submit'>
            New
          </MemoSubmitButton>
        }
        formId='newChild'
      ...
      <ChildMemoCategoryEditorDialog
        ...
        footerButton={
          <MemoSubmitButton form='updatechild' toolTipTitle='submit'>
            Update
          </MemoSubmitButton>
        }
        formId='updatechild'
