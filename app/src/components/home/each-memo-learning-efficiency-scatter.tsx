...
import {
  selectEachMemoLearningEfficiencyOptions,
  fetchAsyncGetSelectMemoLearningEfficiency,
  selectSelectEachParentMemoCategoryLearningEfficiency,
  fetchAsyncPatchLearningEfficiency,
...
interface memoDataset {
  label: string;
  data: memoData[];
  backgroundColor: string;
}

type MemoScatterOption = _DeepPartialObject<
...
export const EachMemoLearningEfficiencyScatter: VFC = () => {
  ...
  const eachParentMemoCategoryLearningEfficiencySelectData = useSelector(
    selectSelectEachParentMemoCategoryLearningEfficiency,
  );
  ...
  const titleItem = (tooltipItem) => {
    const ChildMemoCategoryName =
      datasets[tooltipItem[0].datasetIndex].data[tooltipItem[0].dataIndex].childMemoCategoryName;
  ...
  filterdEachMemoLearningEfficiencyOptions.forEach((memoOption) => {
    const index = datasets.findIndex(
      (dataset) => dataset.label === memoOption.childMemoCategoryName,
    );
  ...
  filterdEachMemoLearningEfficiencyOptions.forEach((memoOption, index) => {
    const datasetIndex = datasets.findIndex(
      (data) => data.label === memoOption.childMemoCategoryName,
    );
  ...
  const handleClick = async (event: MouseEvent<HTMLCanvasElement, globalThis.MouseEvent>) => {
    ...
    if (element.length !== 0) {
      ...
      await dispatch(
        setSelectChildMemoCategoryName(element[0].element.$context.raw.childMemoCategoryName),
      );
      ...
      await dispatch(
        fetchAsyncGetEachMemoLearningEfficiency(
          eachParentMemoCategoryLearningEfficiencySelectData.parentMemoCategoryId,
        ),
      );
    }
  };
  ...
  return (
    <>
      ...
      <SelectMemoDialog isOpen={isOpenSelectMemoDialog} onClose={handleOnClose} />
    </>
  );
};
