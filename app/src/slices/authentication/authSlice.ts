...
import type {
  ...
  Nickname,
  NicknamePatchProps,
  ProfileImagePatchProps,
} from '../../types/auth';

const apiUrl = process.env.NEXT_PUBLIC_AKNOTEBOOK_API_URL;

export const fetchAsyncLogin = createAsyncThunk('auth/post', async (authen: AuthProps) => {
...
export const fetchAsyncRegister = createAsyncThunk('auth/register', async (authen: AuthProps) => {
...
export const fetchAsyncCreateInitialUserData = createAsyncThunk(
  ...
  async () => {
    const res = await axios.get(`${apiUrl}api/initial-data/`, {
      headers: {
        'Content-Type': 'application/json',
...
export const fetchAsyncCreateProf = createAsyncThunk('profile/post', async (nickname: Nickname) => {
...
export const fetchAsyncPatchProfileImage = createAsyncThunk(
  ...
  async (profileImage: ProfileImagePatchProps) => {
    ...
    uploadData.append('profileImage', profileImage.profileImage, profileImage.profileImage.name);
    const res = await axios.patch(`${apiUrl}api/profile/${profileImage.profileId}/`, uploadData, {
      headers: {
        'Content-Type': 'application/json',
        Authorization: `JWT ${localStorage.accessToken}`,
      },
