...
export const fetchAsyncGetPurposes = createAsyncThunk('purpose/get', async () => {
  const res = await axios.get(`${apiUrl}api/purpose/`, {
    headers: {
...
export const fetchAsyncGetPurposesFilter = createAsyncThunk(
  'purpose/get/note',
  async (noteId: String) => {
    const res = await axios.get(`${apiUrl}api/filter/purpose/?note=${noteId}`, {
      headers: {
        Authorization: `JWT ${localStorage.accessToken}`,
      },
    });
...
export const fetchAsyncCreatePurpose = createAsyncThunk(
  ...
  async (purpose: PurposeProps) => {
    ...
    const res = await axios.post(`${apiUrl}api/purpose/`, purpose, {
      headers: {
...
export const fetchAsyncPatchPurpose = createAsyncThunk(
  ...
  async (purpose: UpdatePurposeProps) => {
    ...
    const res = await axios.patch(`${apiUrl}api/purpose/${purpose.purposeId}/`, updateData, {
      headers: {
        'Content-Type': 'application/json',
        Authorization: `JWT ${localStorage.accessToken}`,
      },
...
export const fetchAsyncLogicalDeletePurpose = createAsyncThunk(
  ...
  async (purposeId: string) => {
    ...
    const res = await axios.patch(`${apiUrl}api/purpose/${purposeId}/`, updateData, {
      headers: {
        'Content-Type': 'application/json',
        Authorization: `JWT ${localStorage.accessToken}`,
      },
...
export const purposeSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder.addCase(fetchAsyncGetPurposes.fulfilled, (state, action) => {
      state.purposes = action.payload;
    });
    builder.addCase(fetchAsyncGetPurposesFilter.fulfilled, (state, action) => {
      state.purposeOptions = action.payload.results;
    });
    builder.addCase(fetchAsyncCreatePurpose.fulfilled, (state, action) => {
      return {
        ...state,
        purposeOptions: [...state.purposeOptions, action.payload],
      };
    });
    builder.addCase(fetchAsyncPatchPurpose.fulfilled, (state, action) => {
      state.purposeOptions = state.purposeOptions.map((purpose) =>
        purpose.purposeId === action.payload.purposeId ? action.payload : purpose,
      );
    });
    builder.addCase(fetchAsyncLogicalDeletePurpose.fulfilled, (state, action) => {
...
export const { changeEditPurpose, resetEditPurpose } = purposeSlice.actions;
